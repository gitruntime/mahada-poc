...
const AddNewPropertyPage2 = () => {
    ...
    const validateForm = () => {
        ...
        if (!formData.ownershipDate) {
            ...
            if (selectedDate > today) {
                newErrors.ownershipDate = "Date cannot be in the future";
            }
        }
    ...
    return (
        <div className="w-full flex justify-center py-10">
            <section className="w-full max-w-[1240px] bg-white border border-[#E6E8F0] rounded-[20px] px-6 sm:px-10 py-8">
                ...
                {/* HEADER */}
                <h2
                    className={`text-[24px] font-semibold text-[#0A0A0A] mb-6 ${poppins.className}`}
                >
                    Add Property
                </h2>
                ...
                <div className="flex flex-col mb-4 items-center w-full">
                    ...
                    <div className="flex justify-between w-full">
                        {[
                            { icon: "/document_icon.png", label: "Property Info", inactive: true },
                            { icon: "/upload.png", label: "Upload", inactive: true },
                            { icon: "/review_icon.png", label: "Review & Payment", inactive: true },
                        ].map((step, idx) => (
                            <div
                                key={idx}
                            ...
                            >
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center ${step.inactive ? "bg-white" : ""
                                        }`}
                                >
                                    <img
                                        src={step.icon}
                                ...
                                <span
                                    className={`text-[12px] mt-1 ${poppins.className} ${step.inactive
                                        ? "text-gray-400"
                                        : "text-[#028541]"
                                        }`}
                                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mb-8">
                    ...
                    <div>
                        <label
                            className={`block text-[14px] font-medium mb-1 ${poppins.className}`}
                        >
                            Title *
                        </label>
  ...
  onClick={() => {
    if (typeof window !== "undefined") {
      const el = document.getElementById("ownershipDate");
      el?.showPicker?.(); // safely trigger only if available
    }
  }}
...
>
  ...
  >
    ...
    <line x1="3" y1="10" x2="21" y2="10" />
                        ...
                        <input
                            id="ownershipDate"
                            name="ownershipDate" // 👈 renamed
                            type="date"
                            placeholder="Select date"
                            className={`w-full p-2 pl-10 border border-gray-300 rounded-lg bg-gray-50 
    appearance-none [&::-webkit-calendar-picker-indicator]:hidden
    ${errors.ownershipDate ? "border-red-500" : ""} ${poppins.className}`}
                ...
                <div className="mb-8">
                    ...
                    <textarea
                        ...
                        className={`w-full rounded-[14px] border border-[#D4D7E3] bg-[#F7FBFF] px-4 py-3 text-[14px]
                        focus:outline-none focus:border-orange-400 ${errors.address ? "border-red-500" : ""
                            }`}
