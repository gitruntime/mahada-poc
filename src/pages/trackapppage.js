...
import Profilestrack from "@/Components/RentalAgreement/Profilestrack"
        ...
        <div className="min-h-screen flex flex-col">
            ...
            <div className="flex mb-3 flex-col items-center w-full p-6 bg-gray-50">
                ...
                <div className="relative w-full max-w-[850px] h-2 bg-gray-300 rounded-full mb-2">
                    <div className="absolute top-0 left-0 h-2 bg-orange-500 rounded-full" style={{ width: "4%" }}></div>
                </div>
                ...
                <div className="flex justify-between w-full max-w-[850px] mb-6">
                    {[
                        ...
                        { icon: "/document_icon.png", label: "Verified", active: false },
                        { icon: "/personalinfo_icon.png", label: "Approved", active: false },
                    ].map((step, idx) => (
                        <div key={idx} className="flex flex-col items-center text-center">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${step.active ? "text-green-600" : "text-gray-400"}`}>
                                <img src={step.icon} alt={step.label} className="w-5 h-5" />
                            </div>
                            <span className={`text-[12px] mt-1 ${poppins.className} ${step.active ? "text-green-600" : "text-gray-400"}`}>
                                {step.label}
                            </span>
                        </div>
                    ))}
                ...
                {/* Info Text */}
                <div className="w-full max-w-[1360px] bg-[#F7FBFF] border border-gray-200 rounded-md p-4 text-center mb-6">
                    <p className={`text-[14px] ${poppins.className} text-[#0A0A0A]`}>
                        Your application is submitted and assigned to Verification officer - Mumbai District (MH-VO-021).<br />
                        You will be notified on mail & SMS once the application is approved.
                    </p>
                </div>
